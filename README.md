...
> **現在の実装状態**: 個人用としては「**簡易構成**（GAS 直結 + `LINE_USER_ID` ホワイトリスト、署名検証なし）」で運用中。受託納品や公開時には「**推奨構成**（Cloud Functions プロキシで `x-line-signature` を HMAC 検証 + `proxyToken` で GAS と認証）」への改修が必要（`main.gs` の `doPost` に `proxyToken` 検証分岐を追加する小改修）。本リポジトリの `functions/` には推奨構成用のプロキシコードが既に置かれている。
...
| キー | 説明 | 必須 |
|---|---|---|
| `LINE_ACCESS_TOKEN` | LINE Developers で発行したチャネルアクセストークン（長期） | 簡易・推奨どちらも必須 |
| `LINE_USER_ID` | あなた自身の LINE ユーザーID (`U...` で始まる) | 簡易・推奨どちらも必須 |
| `PROXY_SHARED_SECRET` | Cloud Functions プロキシと共有するランダム秘密（`openssl rand -hex 32` 推奨）。Secret Manager と同じ値を入れる | **推奨構成のみ必須**（要 `doPost` 検証実装） |
| `TARGET_CALENDAR_ID` | 対象カレンダーの ID（空にすると主カレンダー） | 任意 |
| `WEATHER_AREA_CODE` | 気象庁エリアコード（任意、デフォルト `130000` = 東京） | 任意 |
| `WEATHER_AREA_LABEL` | 天気予報の表示ラベル（任意、デフォルト `東京`） | 任意 |

`LINE_ACCESS_TOKEN` / `LINE_USER_ID` が未設定だとボットは反応しません。`PROXY_SHARED_SECRET` は簡易構成では未使用なので空のままでも構いません。`TARGET_CALENDAR_ID` は主カレンダーで良ければ空のままで OK。
...
- **署名検証**: 簡易構成では未実装（GAS の `doPost` は HTTP ヘッダを取れない構造的制約のため、`LINE_USER_ID` ホワイトリストで限定）。推奨構成では `functions/` の Cloud Functions プロキシで `x-line-signature` を HMAC 検証する
- 個人用・単一ユーザー前提の設計。家族や他ユーザーにも使わせたい場合（受託納品含む）は推奨構成への改修が必要
...
> **前提**: 受託納品では「推奨構成（Cloud Functions プロキシ + `proxyToken` 検証）」への改修を行う。`main.gs` の `doPost` に `wrapped.proxyToken !== PROXY_SHARED_SECRET` の検証分岐を追加する小改修が必要（現行の `doPost` は LINE 生 body をそのまま受ける構造）。
...
### 引き渡し後の必須作業（東城が値を保持しない運用にするため）

引き渡しが終わったら、以下をクライアント側で実施し、東城は値を知らない状態にする:

1. LINE Channel Access Token を**再発行**（旧トークンを失効）
2. `PROXY_SHARED_SECRET` を**再生成**して Secret Manager と GAS Properties の両方を更新
3. Secret Manager IAM / GAS スクリプト共有から東城のアカウントを**外す**
4. 必要に応じて `LINE_USER_ID` をクライアント側のユーザー ID に置き換える
