...
- **夜の通知** — 毎晩20時に、翌日の予定と前夜リマインダーを配信
- **前夜リマインダー** — カレンダーに `【リマインド】○○` というタイトルで登録しておくと、前日夜にそのリマインダーが通知される（ごみ出し・定期的な買い物など）
...
| 何もしない（毎晩20時） | 翌日の予定と前夜リマインダーを自動配信 |
...
| `【目標】○○` | 朝通知でカウントダウン表示される |
| `【タスク】○○` | 期限3日前から朝通知に出る |
| `【リマインド】○○` | 前日夜に「明日のリマインダー」として通知される（予定一覧には出ない） |
